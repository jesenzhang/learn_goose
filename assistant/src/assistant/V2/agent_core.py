...
# --- Core Tools ---
DEEP_THINKING_HEADER = "--- SYSTEM OVERRIDE: DEEP THINKING PROTOCOL ---"
...
def _strip_deep_thinking_instruction(text: str) -> tuple[str, bool]:
    """Remove deep thinking instructions from text defensively."""
    if not text:
        return text, False
    changed = False
    if DEEP_THINKING_INSTRUCTION in text:
        text = text.replace(DEEP_THINKING_INSTRUCTION, "")
        changed = True
    if DEEP_THINKING_HEADER in text:
        text = text.split(DEEP_THINKING_HEADER)[0].rstrip()
        changed = True
    return text, changed

def update_plan(steps: List[str], _ctx=None) -> str:
    return f"PLAN_UPDATED:{json.dumps(steps, ensure_ascii=False)}"
...
class MicroAgentV2Core:
    ...
    async def _emit_event(
        ...
        *,
        ...
        if not session_id or not run_id:
            raise RuntimeError("emit_event requires session_id and run_id")
        metadata = {
            "user_id": user_id,
        }
    ...
    async def _handle_intent_phase(
        self,
        user_input: str,
        state: AgentState,
        gen: AgentGeneration,
        *,
        ...
        if not state.intent_queue:
            ...
            if results.intents:
                ...
                await self._emit_event(
                    EventType.STATE_CHANGE,
                    {"msg": "📋 Plan Formulated", "plan": plan_desc, "label": "计划已生成"},
                    session_id=state.session_id,
                    run_id=run_id,
                    user_id=user_id,
                )
        ...
        await self._emit_event(
            EventType.STATE_CHANGE,
            {"msg": f"🚀 Executing: {primary.intent}", "slots": primary.entities, "label": f"执行中:{primary.label}"},
            session_id=state.session_id,
            run_id=run_id,
            user_id=user_id,
        )
...
"""
    ...
    def _build_system_prompt(self, state: AgentState, gen: AgentGeneration, req_ctx: RequestContext) -> str:
        ...
        if state.current_plan:
            prompt += f"\nPlan: {json.dumps(state.current_plan, ensure_ascii=False)}"
        ...
        *,
        ...
        async def _exec_wrapper(index: int, req: ToolRequest) -> tuple:
            ...
            await self._emit_event(
                ...
                {"name": name, "args": args, "label": tool_label},
                session_id=state.session_id,
                run_id=run_id,
                user_id=user_id,
            )
            ...
            # 2. 执行工具逻辑
            try:
                tool_result = await self._exec_tool_func(
                    name,
                    args,
                    state,
                    gen,
    ...
    async def run_task(
        ...
        logger.debug(
            "run_task start: session_id=%s resume=%s user_id=%s input_type=%s",
            state.session_id,
            resume,
            user_id,
        ...
        if handle and handle.is_running:
            logger.info(f"Task for session {state.session_id} is already running [RunID: {handle.task_id}]")
            return handle
        ...
        logger.debug(
            "run_task scheduled: session_id=%s run_id=%s",
            state.session_id,
            current_run_id,
        )
    ...
    async def _task_wrapper(
        ...
        """
        logger.debug(
            "task_wrapper start: session_id=%s run_id=%s user_id=%s",
            session_id,
            task_id,
            user_id,
        )
        try:
            ...
            if state is not None:
                ...
                self._schedule_state_save(session_id, state)
            self._running_tasks.pop(session_id, None)
            logger.debug(
                "task_wrapper end: session_id=%s run_id=%s",
                session_id,
                task_id,
            )
    ...
    async def _run_task_body(
        ...
        logger.debug(
            "task_body start: session_id=%s run_id=%s resume=%s user_id=%s",
            session_id,
            task_id,
            resume,
            user_id,
        ...
        async with current_gen_ref.context_scope() as gen:
            try:
                # 1. 使用注入的状态
                if state is None:
                    raise RuntimeError("AgentState is required")
                logger.debug(
                    "state injected: session_id=%s status=%s history_len=%s user_id=%s",
                    state.session_id,
                    state.status,
                ...
                emit_kwargs = {"session_id": session_id, "run_id": task_id, "user_id": user_id}
                ...
                if isinstance(input_data, dict):
                    ...
                    logger.debug(
                        "request context: server_type=%s deep_thinking=%s deep_research=%s has_file=%s has_page=%s",
                        req_ctx.server_type,
                        req_ctx.deep_thinking,
                        req_ctx.is_deep_research,
                        bool(req_ctx.file_path),
                        bool(req_ctx.page_content),
                    )
                else:
                ...
                if resume and state.status == AgentStatus.WAITING_APPROVAL:
                    ...
                    approved = await self._process_approval(
                        state,
                        approval_data or {},
                        gen,
                        run_id=task_id,
                    ...
                    logger.debug(
                        "approval processed: session_id=%s approved=%s",
                        session_id,
                        approved,
                    )
                ...
                # 4. 如果是新输入，处理 Hook 和 意图
                    ...
                    logger.debug(
                        "user input received: session_id=%s len=%s",
                        session_id,
                    ...
                    if self.hook_manager:
                        ...
                        if hook_res and hook_res.action == HookAction.INTERCEPT:
                            logger.debug(
                                "hook intercept: session_id=%s action=%s",
                                session_id,
                                hook_res.action,
                            )
                            await self.add_message(session_id, Message.assistant(hook_res.response), state)
                            await self._emit_event(EventType.TOKEN, hook_res.response, **emit_kwargs)
                            state.status = AgentStatus.IDLE
                            return

                    # --- Intent Analysis ---
                    state.status = AgentStatus.RUNNING
                    direct_res = await self._handle_intent_phase(
                        user_input,
                        state,
                        gen,
                        run_id=task_id,
                    ...
                    logger.debug(
                        "intent phase complete: session_id=%s direct_res=%s",
                        session_id,
                        bool(direct_res),
                    )
                ...
                logger.debug(
                    "llm loop start: session_id=%s run_id=%s",
                    session_id,
                    task_id,
                )
                while state.status == AgentStatus.RUNNING:
                    ...
                    # Defensive check: if deep_thinking is False, strip any injected instruction.
                    if not req_ctx.deep_thinking:
                        ...
                        if removed:
                            logger.warning(
                                "deep_thinking_instruction removed from input_messages (deep_thinking=False) session_id=%s",
                                session_id,
                            )
                        else:
                            logger.debug(
                                "deep_thinking_instruction not found in input_messages (deep_thinking=False) session_id=%s",
                                session_id,
                            )
                    ...
                    thinking = ThinkingTracker(
                        lambda et, data: self._emit_event(et, data, **emit_kwargs)
                    )
                    async with self.event_scope(
                        ...
                        **emit_kwargs,
                        ...
                        try:
                                ...
                                if partial_msg and partial_msg.content:
                                    for c in partial_msg.content:
                                        ...
                                        # =================================================
                                            ...
                                            if not req_ctx.deep_thinking:
                                                await thinking.end()
                                                await self._emit_event(EventType.TOKEN, c.text, **emit_kwargs)
                                            ...
                                            for char in text_chunk:
                                                ...
                                                if parse_state == "normal":
                                                    if char == "<":
                                                        ...
                                                        await self._emit_event(EventType.TOKEN, char, **emit_kwargs)
                                                ...
                                                # --- 状态 2: 检查开始标签 <thinking> ---
                                                    ...
                                                    if tag_buffer == "<thinking>":
                                                        ...
                                                        await self._emit_event(EventType.TOKEN, tag_buffer, **emit_kwargs)
                                                        parse_state = "normal"
                                                        tag_buffer = ""
                            ...
                            await thinking.end()                        
                            logger.debug(
                                "llm stream complete: session_id=%s run_id=%s text_len=%s tool_requests=%s duration_ms=%s",
                                session_id,
                                task_id,
                                len(full_content_text),
                                len(received_tool_requests),
                    ...
                    if received_tool_requests:
                        ...
                        logger.debug(
                            "tool requests enqueued: session_id=%s count=%s",
                            session_id,
                            len(received_tool_requests),
                        )
                    ...
                    if not received_tool_requests:
                        ...
                        self._schedule_state_save(state.session_id, state)
                        logger.debug(
                            "llm loop end (no tools): session_id=%s",
                            session_id,
                        )
                        break # Turn complete
                    ...
                    exec_results = await self._execute_tools_concurrent(
                    ...
                    logger.debug(
                        "tools executed: session_id=%s count=%s duration_ms=%s",
                        session_id,
                        len(exec_results),
                    ...
                    self._schedule_state_save(state.session_id, state)
                    logger.debug(
                        "state scheduled save: session_id=%s status=%s",
                        session_id,
                        state.status,
                    )
                ...
                logger.debug(
                    "task_body end: session_id=%s run_id=%s status=%s",
                    session_id,
                    task_id,
    ...
    async def _process_approval(
        ...
        *,
        ...
        if data.get("approved"):
            # 传递 gen
            res = await self._exec_tool_func(
                tool["name"],
                tool["args"],
                state,
                gen,
    ...
    async def cancel_task(self, session_id: int) -> bool:
        ...
        try:
            ...
            await self._emit_event(
                EventType.CANCELLED,
                {"msg": "Task cancelled by user", "session_id": session_id},
                session_id=session_id,
                run_id=f"cancel_{session_id}",
