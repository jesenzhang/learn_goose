...
@register_store(StoreType.REMOTE)
class RemoteMemoryStore(Store):
    def __init__(self, session_id: str, config: Optional[StoreConfig] = None, **kwargs):
        self.session_id = str(session_id)
        self.config = config or StoreConfig(store_type=StoreType.REMOTE)
        ...
        self.search_path = kwargs.get("search_path", "/memory/search")
    ...
    async def load_lines(self, ref: StoreRef, start: int, limit: int) -> List[str]:
        ...
        if isinstance(data, str):
            return data.splitlines()[start : start + limit]
        return []
    ...
    async def search(self, ref: StoreRef, pattern: str, max_hits: int = 20) -> List[str]:
        ...
        resp = await self._post(self.search_path, payload)
        if not resp:
            return []
        hits = resp.get("hits")
        if isinstance(hits, list):
            return [str(hit) for hit in hits]
        return []
    ...
    def _serialize(self, ref: StoreRef, data: Any) -> Dict[str, Any]:
        return {
            "session_id": self.session_id,
            "item_id": ref.id,
            "type": ref.type,
