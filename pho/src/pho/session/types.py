...
class Session(BaseModel):
    ...
    多用户支持：
    - user_id: 会话所有者的用户 ID
    - 通过 session_collaborators 表支持多用户协作
    """
    id: str
    ...
    # 多用户支持
    user_id: Optional[str] = Field(default=None, description="（用户 ID）会话所有者的用户 ID")

    # 1. 通用元数据：存放 working_dir 等
    metadata: Dict[str, Any] = Field(default_factory=dict, description="（元数据）存放 working_dir 等通用元数据")
