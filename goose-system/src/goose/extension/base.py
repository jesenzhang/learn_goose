...
class FrontendExtension(Extension):
    ...
    def __init__(self, config: FrontendExtensionConfig):
        ...
        self._frontend_tools: Dict[str, Dict[str, Any]] = {}
    ...
    async def initialize(self) -> None:
        ...
        for tool in self.tools_config:
            ...
            if tool_name:
                self._frontend_tools[tool_name] = tool
        self._initialized = True
    ...
    async def call_tool(
        ...
        """调用前端工具"""
        if tool_name not in self._frontend_tools:
            raise ValueError(f"Tool not found: {tool_name}")
        
        tool = self._frontend_tools[tool_name]
        ...
        return {"content": [], "isError": True, "error": "No handler configured"}
    
    async def close(self) -> None:
        """关闭 Extension"""
        self._frontend_tools.clear()
        self._initialized = False
...
class ExtensionFactory:
    ...
    def create(config: ExtensionConfig) -> Extension:
        ...
        ExtensionConfig::Platform { name, ... } → PlatformExtension (TODO)
        ...
        if isinstance(config, StdioExtensionConfig):
            ...
            raise NotImplementedError("PlatformExtension not implemented yet")
        else:
