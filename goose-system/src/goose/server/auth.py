...
class AuthMiddleware:
    ...
    3. User-based authentication (session-based)
    """
    ...
    def __init__(
        ...
        Args:
            ...
            auth_mode: Authentication mode
                ...
                - "user": Require user session authentication
            user_manager: User manager for session-based authentication
        """
    ...
    async def validate_request(self, request: Request) -> str:
        ...
        Returns:
            The validated credential type ("secret_key", "bearer", or "user")
        ...
        if self.auth_mode == "user" and self.user_manager:
            session_id = request.headers.get("X-Session-ID")
            if not session_id:
                raise HTTPException(
                    status_code=status.HTTP_UNAUTHORIZED,
                    detail="Missing session ID. Provide X-Session-ID header",
                )

            user = await self.user_manager.get_user_from_session(session_id)
            if not user:
                raise HTTPException(
                    status_code=status.HTTP_UNAUTHORIZED,
        ...
        # Secret key mode only
        if self.auth_mode == "secret_key":
            secret_key = request.headers.get("X-Secret-Key")
            if not secret_key or not self.validate_secret_key(secret_key):
                raise HTTPException(
                    status_code=status.HTTP_UNAUTHORIZED,
        ...
        if self.auth_mode == "bearer":
            ...
            if not self.validate_bearer_token(auth_header):
                raise HTTPException(
                    status_code=status.HTTP_UNAUTHORIZED,
        ...
        # Any mode (default) - accept either X-Secret-Key or Bearer token
...
def create_auth_dependency(
    ...
    Args:
        ...
        auth_mode: "any", "secret_key", "bearer", or "user"
        user_manager: User manager for session-based auth

    Returns:
        Dependency function that validates authentication
    """
    async def verify_credentials(request: Request) -> str:
        auth = AuthMiddleware(secret_key, token, auth_mode, user_manager)
...
async def get_authenticated_credentials(request: Request) -> dict:
    ...
    Returns dict with:
    - credential_type: "secret_key", "bearer", or "user"
