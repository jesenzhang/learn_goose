...
class WorkflowConverter:
    """
    [Compiler] 将静态工作流定义转换为可执行的运行时图 (Graph)。
    适配基于 Component 的新架构。
    """
    ...
    def convert(self, definition: WorkflowDefinition) -> Graph:
        ...
        # 1. 转换节点 (Nodes)
        for node_def in definition.nodes:
            self._add_node_to_graph(graph, node_def)

        # 2. 转换连线 (Edges)
        # 新架构下，Scheduler 会自动处理条件分支，
        # 所以我们只需要把边里的 handle 信息原样存入 Graph 即可。
        ...
        # 3. 设置入口点
        # 策略：优先找 type="start" 的节点，找不到则找没有入边的节点
        start_node = next((n for n in definition.nodes if n.type == "start"), None)
        if start_node:
            graph.set_entry_point(start_node.id)
        else:
            # 简单的 DAG 自动推断入口 (可选)
            all_targets = set(e.target for e in definition.edges)
            potential_starts = [n.id for n in definition.nodes if n.id not in all_targets]
            if potential_starts:
                graph.set_entry_point(potential_starts[0])

        return graph

    def _add_node_to_graph(self, graph: Graph, node_def: NodeConfig):
        """
        根据节点类型，从注册表中加载组件，并配置好参数。
        """
        # 1. 从注册表查找组件类
        component_cls = ComponentRegistry.get(node_def.type)
        
        if not component_cls:
            logger.error(f"❌ Unknown component type '{node_def.type}' for node '{node_def.id}'. Skipping.")
            return

        # 2. 实例化组件
        # 注意：组件通常是无状态的单例，但在 Workflow 中每个节点可能有不同的 Config。
        # 这里我们为每个节点创建一个新的组件实例。
        try:
            component_instance: Component = component_cls()
            
            # 3. 注入配置和元数据
            # Component.invoke 执行时需要用到 config。
            # 我们直接把 config 字典挂载到实例上，或者 Graph 节点上。
            # Goose 的 Scheduler 从 Graph 获取节点后，会读取这些属性。
            
            component_instance.node_id = node_def.id
            component_instance.node_type = node_def.type
            
            # [关键] 保存配置数据 (dict)
            # execute 时会用 Pydantic model 来解析它
            component_instance.config = node_def.config 
            
            # [关键] 保存输入参数映射 (inputs)
            # 例如: {"url": "{{ start.url }}"}
            # 现在的 Scheduler 在 invoke 前会读取它并进行 PromptEngine 渲染
            # 这里我们把 inputs 字典直接作为 graph 节点的属性之一，
            # 但为了方便，我们暂且把 component 实例本身当作载体。
            
            # 注意：Component 基类中没有 inputs 属性，
            # 我们可以动态绑定，或者修改 Graph 的存储结构。
            # 这里简单起见，我们在 NodeConfig 里的 inputs (dict) 就是我们需要的东西。
            # Scheduler 会读取 node_config.inputs。
            
            # 实际上，Scheduler 是通过 graph.get_node(id) 拿到这个 component_instance 的。
            # 所以我们需要把 node_def 的 inputs 存到 component_instance 上。
            # 更好的做法是 Graph 节点存储一个包装器，但为了兼容性：
            setattr(component_instance, "inputs", node_def.inputs)

            # 4. 添加到图
            graph.add_node(node_def.id, component_instance)
            
            logger.debug(f"✅ Loaded node {node_def.id} ({node_def.type})")

        except Exception as e:
            logger.error(f"Failed to instantiate node {node_def.id}: {e}")
            raise e